...
<body>
    <section class="bg-[#f2f2f2] h-screen w-full flex justify-center items-center">
        <x-form method="POST" action="{{ route('register') }}">
            ...
            <x-slot:input>
                <x-form-input id="name" name="name" placeholder="Buat Username" autofocus required>
                    <x-slot:label_for>name</x-slot:label_for>
                    Username
                </x-form-input>
                <x-form-input type="email" id="email" name="email" placeholder="Masukkan Email" required>
